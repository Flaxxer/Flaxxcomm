...
def Get():
	...
	m3u  = 'http://nipsatus.com:25461/get.php?username='+username+'&password='+password+'&type=m3u_plus&output=ts'
	epg  = 'nipsatus.com:25461/xmltv.php?username='+username+'&password='+password
	auth = 'nipsatus.com:25461/enigma2.php?username='+username+'&password='+password+'&type=get_vod_categories'
	...
	if not auth=="":
		...
		xbmcgui.Dialog().ok('wmc','[COLOR blue]M3U URL: [/COLOR]%s'%shortm3u,'','[COLOR blue]EPG URL: [/COLOR]%s'%shortepg)
	else:
		return